...
The goal of LanduseHanneProblem is to generate an interesting model with
the Vilhelmsborg area for this purposes
...
- $SpatialContiguityBonus$: Bonus for adjacent cells with the same
  landuse.
...
$\begin{align*} \text{Maximize } & \text{ConservationIndex} \\ & = \sum_{l \in \text{Landuses}} \sum_{c \in \text{Cells}} \text{LanduseDecision}_{l,c} \cdot \text{Richness}_{l,c} \cdot \text{PhyloDiversity}_{l,c} \\ & \quad + \text{SpatialContiguityBonus} \cdot \sum_{(i,j) \in E} \sum_{l \in \text{Landuses}} \text{LanduseDecision}_{l,i} \cdot \text{LanduseDecision}_{l,j} \end{align*}$
...
of richness, phylogenetic diversity, and the decision variable, which
has a bonus if adjacent cells are of the same landuse.
...
Minimum Cells per landuse Contraint ensures that we at least have a
certain number of each landuse
